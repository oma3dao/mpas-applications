...
MPAS bridge MCP server for **mongodb**, generated by `bridge-generator` from upstream `MongoDB MCP Server`.

Tools: aggregate-db, aggregate, collection-indexes, collection-schema, collection-storage-size, connect, count, create-collection, create-index, db-stats, delete-many, drop-collection, drop-database, drop-index, explain, export, find, insert-many, list-collections, list-databases, mongodb-logs, rename-collection, update-many, atlas-local-connect-deployment, atlas-local-create-deployment, atlas-local-delete-deployment, atlas-local-list-deployments, list-knowledge-sources, search-knowledge
