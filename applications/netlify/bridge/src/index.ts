 ...
 * Upstream: npx -y mcp-remote@0.1.38 https://netlify-mcp.netlify.app/mcp
 *
...
interface CliConfig {
  ...
  applicationDid?: string;
  adapterUrl?: string;
  actionEndpoint?: { url?: string; verifierDid?: string; additionalRecipients?: string[] };
  agentKey?: string;
  executionProfile?: {
    id: string;
    format: string;
...
function relayActionEndpoint(config: RelayActionEndpointConfig, keyManagerPromise: Promise<KeyManager>): WorkflowActionEndpoint {
  ...
  return {
    async submitActionRequest(request: ActionRequest) {
      ...
      return client.submitActionRequest(buildDeliveryEnvelope({ sender: keyManager.did, recipients, payload: request }));
    },
  };
}
...
function toBridgeConfig(config: CliConfig, configDir: string): BridgeConfig {
  ...
  return {
    ...
    ...(actionEndpointUrl && verifierDid ? { actionEndpoint: { url: actionEndpointUrl, verifierDid: verifierDid as Did, additionalRecipients: (config.actionEndpoint?.additionalRecipients ?? []) as Did[] } } : {}),
